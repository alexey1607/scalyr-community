...
The number of simultaneous connections cannot exceed Nginx's limit on the maximum number of open files, configured using the [`worker_rlimit_nofile`](http://nginx.org/en/docs/ngx_core_module.html#worker_rlimit_nofile) directive.  This includes client connections and upstream server connections.  By monitoring the number of open files relative to the limit, you'll have an idea of how much headroom your system has for additional connections.

  * **Monitor** the number of open file handles for each process.
  * **Set an alert** when the number of open file handles reaches 70% of the [`worker_rlimit_nofile`](http://nginx.org/en/docs/ngx_core_module.html#worker_rlimit_nofile) limit.  This means it's time to increase the limit before it reaches capacity and connections are dropped.
